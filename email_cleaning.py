...
async def parse_email(session, email_thread):
    ...
    try:
        async with session.post('https://api.openai.com/v1/chat/completions', json=json_body, headers=headers, timeout=timeout) as response:
        ...
        print(f"Timeout for email: {email_thread[:50]}")
        return "timeout"  # Return a special flag for timeouts
            ...
            for row in csv_reader:
                ...
                if len(email_batch) >= batch_size:
                    ...
                    for email_thread, json_string in zip(email_batch, results):
                        if json_string == "timeout":
                            print(f"Retrying email due to timeout: {email_thread[:50]}")
                            retry_result = await parse_email(session, email_thread)
                            # Process retry_result as needed
                        elif json_string:
                            try:
                    ...
                    # Adding a delay between batches to manage rate limits
...
input_csv_path = 'C:/Users/happy/Downloads/inbox-manager-agent-main/inbox-manager-agent-main/No_Advertise_Filtered_Emails_Small.csv'
output_csv_path = 'C:/Users/happy/Downloads/inbox-manager-agent-main/inbox-manager-agent-main/email_pairs.csv'
